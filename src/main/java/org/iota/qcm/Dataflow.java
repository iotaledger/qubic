...
public class Dataflow implements Branch {
  int[] inputLength;
  Site[] latches;
  Site[] body;
  Site[] outputs;
  ...
  public Dataflow(TritBuffer buffer, Source source, Map<Hash, Source> refMap) {
    int i, n;
    inputLength = new int[buffer.nextPositiveInteger()];
    for(i = 0; i < inputLength.length; i++) {
      inputLength[i] = buffer.nextPositiveInteger();
    }
    body = new Site[buffer.nextPositiveInteger()];
    outputs = new Site[buffer.nextPositiveInteger()];
    latches = new Site[buffer.nextPositiveInteger()];

    for(n = 1; n <= inputLength.length + body.length + outputs.length || n < latches.length; n *= 3);

    for(i = 0; i < body.length; i++) {
      body[i] = new Site(buffer, n, source, refMap);
    }
    for(i = 0; i < outputs.length; i++) {
      outputs[i] = new Site(buffer, n, source, refMap);
    }
    for(i = 0; i < latches.length; i++) {
      latches[i] = new Site(buffer, n, source, refMap);
    }

    prepareInputBuilders();
  }

  public Dataflow(int[] i, Site[] l, Site[] b, Site[] o) {
    inputLength = i;
    latches = l;
    body = b;
    outputs = o;
  ...
  public BranchInstance instance(int depth) {
    BranchInstance b = new BranchInstance(depth);
    return b;
  }
